...
Hassle-free service exposure with automatic LoadBalancer support - works out of the box!

### 🐳 Pull-through Cache via Local Docker Daemon
Faster image pulls and reduced bandwidth by leveraging your local Docker daemon's image cache.
...
- vCluster CLI v0.31.0-rc.7 or later
...
```bash
# Start vCluster Platform in Docker (optional but recommended)
...
experimental:
  docker:
    ...
    # Enable registry proxy for faster pulls
    registryProxy:
      enabled: true
